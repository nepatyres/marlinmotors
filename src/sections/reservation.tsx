...
export default function Reservation() {
    ...
    const handleTypeClick = (index: number) => {
        ...
        calculateSum(services, toggleStates, multiplier);
    };
    ...
    const handleOptionOrToggle = (type: 'option' | 'toggle', params: { option?: string, price: number, selectionIndex?: number, optionIndex?: number, parentIndex?: number, toggleIndex?: number }) => {
        ...
        const totalSum = (serviceSum + toggleSum) * carType;
        setSum(totalSum);
    };
    ...
    const calculateSum = (servicesArray: { price: number }[], toggleArray: { [parentIndex: number]: { [toggleIndex: number]: boolean } }, multiplier: number) => {
        ...
        setSum((totalServices + totalToggles) * multiplier);
    };
                ...
                <div className="flex w-full justify-center">
                    <div className="xl:w-10/12 rounded-md grid lg:grid-cols-2">
                        <ReservLeftSide carTypes={carTypes} handleTypeClick={handleTypeClick} selectedType={selectedType} services={services} selection={selection}
                            ...
                            selectedStates={selectedStates} handleOptionClick={handleOptionClick} handleToggle={handleToggle} toggleStates={toggleStates} />
                        <ReservRightSide toggler={toggler} services={services} selection={selection} handleOptionClick={handleOptionClick} toggleStates={toggleStates}
                            handleToggle={handleToggle} sum={sum} handleTypeClick={handleTypeClick} setReservPopup={setReservPopup} />
                    </div>
