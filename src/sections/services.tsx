...
export default function Services({ language }) {
    ...
    return (
        <div className="flex flex-col items-center bg-gradient-to-b from-black to-[#141414] min-h-screen overflow-hidden" id="scrollToServices">
            <span className={`text-[30px] mt-[50px] lg:mt-[60px] lg:text-[40px] xl:mt-20 uppercase text-white/70 text-center ${language ? 'font-gruppo font-semibold' : 'font-montserratR'}`}>{language ? 'Mūsų paslaugos' : 'Наши услуги'}</span>
                ...
                <div className='mt-[30px] grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 xl:mt-20 gap-4 xl:gap-0 tranform-all ease-in duration-500'>
                    {services.map((service, i) => (
                        i < 5 ?
                            ...
                            (
                                <a href='/reservation' className={`relative w-[250px] h-[250px] sm:w-[230px] sm:h-[230px] md:w-[300px] md:h-[300px] lg:w-[300px] lg:h-[300px] 2xl:w-[350px] 2xl:h-[350px] border border-white/10 mb-4 sm:mr-5 overflow-hidden rounded-md`}>
                                    ...
                                    <div className='absolute flex flex-row inset-0 justify-center items-center text-center pointer-events-none'>
                                        <div className=" text-white font-ubuntuL text-wrap xl:text-[25px] text-[20px]">{language ? service.front : service.frontRu}</div>
