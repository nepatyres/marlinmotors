        ...
        <div id='scrollToAboutUs' className="h-auto pb-10 sm:pb-20 xl:pb-20 overflow-hidden bg-gradient-to-b from-[#141414] to-black">
            <div className="flex-row lg:flex-row w-[90%] mx-auto grid grid-cols-12">
                <div className="aboutus-main xl:col-span-7 col-span-12">
                    <span className={`flex justify-center text-[30px] sm:text-[40px] uppercase text-white/80 pt-20 ${language ? 'font-gruppo font-semibold' : 'font-montserratR'}`}>{language ? 'Apie mus' : 'О нас'}</span>
                ...
                <div className="aboutus-ourvalues xl:pl-[30px] xl:col-span-5 col-span-12 xl:pb-[50px]">
                    <span className={`flex justify-center text-[30px] sm:text-[40px] uppercase text-white/80 pt-20 mb-6 xl:pb-[30px] ${language ? 'font-gruppo font-semibold' : 'font-montserratR'}`}>{language ? 'Mūsų vertybės' : 'Наши ценности'}</span>
                    {aboutUs.map((about, i) => (
                        <div key={i} ref={el => { refs.current[i] = el }} className='flex flex-row mt-3 items-center mb-10 sm:mb-0'>
                            ...
                            <div className="flex flex-col ml-[10px] sm:ml-[30px]">
                                <span className="text-white/70 text-[20px] sm:text-[25px] font-ubuntuL">{language ? about.span : about.spanRu}</span>
                                <p className="text-white/40 text-[15px] text-wrap">{language ? about.text : about.textRu}</p>
                            </div>
                        </div>
                    ))}
                </div>
