...
export default function ReservPopup({ language, setReservPopup, sum }: any) {
    ...
    return (
        <div className="flex fixed inset-0 min-h-full w-full z-[9998] backdrop-blur-sm items-center justify-center">
            <div className="p-0 m-0 rounded-2xl w-[380px] sm:w-[500px] h-[700px] items-end flex-col flex bg-black">
                ...
                <div className="flex flex-col text-center mt-4 mx-auto">
                    ...
                    <button onClick={() => calPopupBtn()} disabled={!isFormComplete} className={`w-60 sm:w-80 mt-10 self-start h-10 font-bold border-none rounded-md text-base cursor-pointer bg-slate-200 text-[#182f3d] hover:bg-slate-300 transition-colors duration-500 mb-10  ${isFormComplete ? "" : "cursor-not-allowed"}`}>{language ? 'Toliau' : 'Дальше'}</button>
                </div >
            </div>
            {
            calPopup && 
            <CalPopup language={language} setCalPopup={setCalPopup} setReservPopup={setReservPopup} formData={formData} />}
        </div>
    )
}