            ...
            <nav className={`fixed left-0 w-full top-0 h-[60px] z-50 flex justify-center ${scrolling ? 'backdrop-blur-md bg-black/20' : ''}`} id="nav-blur">
                <div className="w-[95%] 2xl:max-w-[80%] grid grid-cols-[1fr_1fr] lg:grid-cols-[1fr_2fr_1fr] m-auto">
                    ...
                    <div className="hidden lg:flex flex-row justify-center items-center gap-5">
                        <a href={'/rezervacija'} className="text-white font-UbuntuR text-[20px] tracking-wide cursor-pointer transition-colors ease-in duration-500 hover:text-[#c1d8f0]">{language ? 'Kainos' : 'Цены'}</a>
                        {navbar.map((nav, i) => (
                            <a key={i} onClick={() => scrollTo(nav.scrollT)} className="text-white font-UbuntuR text-[20px] tracking-wide cursor-pointer transition-colors ease-in duration-500 hover:text-[#c1d8f0]">{language ? nav.nameLt : nav.nameRu}</a>
                        ))}
                    </div>
            ...
            {navbarVisible && (
                <div className='fixed lg:hidden w-full top-[60px] bg-black/20 z-50 backdrop-blur-md' id="navbarNav">
                    <ul className='text-center items-center'>
                        <a href={'/rezervacija'} className='text-white font-gruppo font-medium text-[20px] tracking-wide cursor-pointer transition-colors ease-in duration-500 hover:text-[#c1d8f0]'>{language ? 'Kainos' : 'Цены'}</a>
                        {navbar.map((nav, index) => (
                            <li key={index} className="pt-3">
                                <a onClick={() => scrollTo(nav.scrollT)} className='text-white font-UbuntuR font-light text-[20px] tracking-wide cursor-pointer transition-colors ease-in duration-500 hover:text-[#c1d8f0]'>{language ? nav.nameLt : nav.nameRu}</a>
                            </li>
                        ))}

                        <li className="pt-3 justify-center flex pb-3">
                            <a onClick={() => scrollTo('scrollToFooter')} className='text-white font-UbuntuR font-light text-[20px] tracking-wide cursor-pointer transition-colors ease-in duration-500 hover:text-[#c1d8f0]'>
                                {phoneSvg}
                            </a>
                            <a className='text-white font-gruppo font-medium text-[20px] tracking-wide cursor-pointer transition-colors ease-in duration-500 hover:text-[#c1d8f0]'>
                                LT
                            </a>
                        </li>
                    </ul>
