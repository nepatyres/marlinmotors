...
export default function ReservLeftSide({ language, carTypes, accordionStates, setAccordionStates, handleTypeClick, selectedType, selection, dropdownRefs, handleOptionClick, toggleState, toggleStates, handleToggle, selectedOption, selectedStates }: any) {
                            ...
                            {accordionStates[i] && (
                                <div className={`gap-5 ${i > 1 ? '' : 'pt-3'}`}>
                                    ...
                                    {s.toggle.map((tog: any, idx: number) => {
                                        const isVisible = i !== 0 ||
                                            (
                                                selectedOption[i] !== s.options[2].name && selectedOption[i] !== s.options[2].nameRu &&
                                                (
                                                    selectedOption[i] === s.options[0].name || selectedOption[i] === s.options[0].nameRu ||
                                                    (selectedOption[i] === s.options[1].name || selectedOption[i] === s.options[1].nameRu ? idx === 0 : true)
                                                )
                                            );
                                        const secondIsVisible = i !== 3 ||
                                            (idx !== 0 || !selectedOption[1] || selectedOption[1] === (language ? 'Pasirinkite paslaugą' : 'Выберите услугу')) &&
                                            (
                                                selectedStates[idx] !== 0 || (selectedOption && tog[0] ? false : true)
                                            );
                                        return isVisible && secondIsVisible ? (
                                            <div key={idx} className="flex flex-row items-center mt-2 px-1">
                                                {i !== 2 && <div className="relative flex items-center flex-shrink-0">
                                                    ...
                                                    {priceCheck === `${i}-${idx}` && (
                                                        <div className="absolute left-0 top-full mt-2 bg-black/80 text-white p-2 rounded z-30">{language ? 'Nuo' : 'От'} €{tog.price}</div>
                                                    )}
                                                </div>}
